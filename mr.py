...
from fastprompt_utils import _translate_cuda_visible_devices, _sentence_count_plan, append_csv_row, _ensure_dir, _parse_int_list, _safe_std, _now_tag
...
def parse_args():
    ...
    p.add_argument("--infer-subsample-size", type=int, default=30, help="Subsample from mr_infer per crafting iteration.")
    ...
    p.add_argument("--save-examples-to", type=str, default="mr_proposed_examples.txt", help="Where to save initial generated examples.")
    p.add_argument("--save-crafted-examples-to", type=str, default="mr_crafted_examples.txt", help="Where to save crafted examples.")
    ...
    p.add_argument("--grid-results-csv", type=str, default="results/mr_initial.csv", help="Where to save grid/random search results CSV.")
    ...
    p.add_argument("--shapley-log-dir", type=str, default="results_shapley_mr", help="Directory to save per-iteration Shapley CSV logs.")
        ...
        for spec in unique_specs:
            ...
            for idx, seed in enumerate(ordered_seed_list):
                ...
                if this_row_early_stop:
                    print(f"[RANDOM][EARLY-STOP] spec#{spec_id}: seed=1 acc={acc:.4f} < 0.68 — skipping remaining seeds for this spec.")
                    early_stopped = True
                    break
