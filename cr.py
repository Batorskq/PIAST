...
def parse_args():
    ...
    p.add_argument("--dataset-path", type=str, default="dataset/cr_test.jsonl", help="Path to cr_test.jsonl.")
    p.add_argument("--infer-dataset-path", type=str, default="dataset/cr_infer.jsonl", help="Path to cr_infer.jsonl.")
    p.add_argument("--infer-subsample-size", type=int, default=30, help="Subsample from cr_infer per crafting iteration.")
    p.add_argument("--seed", type=int, default=123, help="Random seed base for subsampling and proposal generation.")
    ...
    p.add_argument("--save-examples-to", type=str, default="cr_proposed_examples.txt", help="Where to save initial generated examples.")
    p.add_argument("--save-crafted-examples-to", type=str, default="cr_crafted_examples.txt", help="Where to save crafted examples.")
    ...
    p.add_argument("--grid-results-csv", type=str, default="results/cr_initial.csv", help="Where to save grid/random search results CSV.")
    ...
    p.add_argument("--shapley-log-dir", type=str, default="results_shapley_cr", help="Directory to save per-iteration Shapley CSV logs.")
    p.add_argument("--random-shapley-permutations", type=str, default="1,2,3,5,10", help="Comma-separated values for Shapley permutations to sample in random search.")
    return p.parse_args()
...
def _build_label_specific_messages(
    ...
    for i in range(num_examples):
        n = counts[i]
        plan_lines.append(
            f"- Example{i+1}: write exactly {n} sentence{'s' if n>1 else ''}; "
    ...
    sys_msg = (
        ...
        "for *binary sentiment* on short movie-review style snippets."
    )
...
LABEL_RE = re.compile(r'^\s*Label\s*:\s*(positive|negative)\b', re.IGNORECASE | re.MULTILINE)
...
class VLLMProposer:
    ...
    def _gen_with_messages(self, messages, max_new_tokens: int, seed: Optional[int] = None) -> str:
        prompt_text = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        from vllm import SamplingParams
        sp = SamplingParams(temperature=0.6, top_p=0.9, seed=seed, max_tokens=max_new_tokens, stop=["\n\n\n"])
        return self.llm.generate([prompt_text], sp, use_tqdm=False)[0].outputs[0].text

    def generate_examples_balanced(self, k: int, max_new_tokens: Optional[int] = None, seed: Optional[int] = None) -> Tuple[str, List[Dict[str, str]]]:
        k_pos = k // 2
        k_neg = k - k_pos
        sent_counts = _sentence_count_plan(k)
        pos_counts = [sent_counts[i] for i in range(0, k, 2)][:k_pos]
        neg_counts = [sent_counts[i] for i in range(1, k, 2)][:k_neg]
        if len(pos_counts) < k_pos:
            pos_counts += [sent_counts[-1]] * (k_pos - len(pos_counts))
        if len(neg_counts) < k_neg:
            neg_counts += [sent_counts[-1]] * (k_neg - len(neg_counts))
        if max_new_tokens is None or max_new_tokens <= 0:
            max_new_tokens = max(64, 48 * max(k_pos, k_neg))

        pos_seed = seed
        neg_seed = None if seed is None else seed + 1

        pos_msgs = _build_label_specific_messages(k_pos, "positive", pos_counts, SENTIMENT_TOPICS)
        neg_msgs = _build_label_specific_messages(k_neg, "negative", neg_counts, SENTIMENT_TOPICS)
        pos_text = self._gen_with_messages(pos_msgs, max_new_tokens, seed=pos_seed)
        neg_text = self._gen_with_messages(neg_msgs, max_new_tokens, seed=neg_seed)

        pos_parsed = [ex for ex in parse_examples(pos_text, k_pos) if ex["label"] == "positive"]
        neg_parsed = [ex for ex in parse_examples(neg_text, k_neg) if ex["label"] == "negative"]
        combined = pos_parsed + neg_parsed

        need_pos = k_pos - sum(1 for ex in combined if ex["label"] == "positive")
        need_neg = k_neg - sum(1 for ex in combined if ex["label"] == "negative")

        seed_cursor = None if seed is None else seed + 2
        if need_pos > 0:
            top_msgs = _build_label_specific_messages(need_pos, "positive", pos_counts[:need_pos], SENTIMENT_TOPICS)
            top_text = self._gen_with_messages(top_msgs, max_new_tokens, seed=seed_cursor)
            seed_cursor = None if seed_cursor is None else seed_cursor + 1
            combined += [ex for ex in parse_examples(top_text, need_pos) if ex["label"] == "positive"]
        if need_neg > 0:
            top_msgs = _build_label_specific_messages(need_neg, "negative", neg_counts[:need_neg], SENTIMENT_TOPICS)
            top_text = self._gen_with_messages(top_msgs, max_new_tokens, seed=seed_cursor)
            seed_cursor = None if seed_cursor is None else seed_cursor + 1
            combined += [ex for ex in parse_examples(top_text, need_neg) if ex["label"] == "negative"]
...
def normalize_label(text: str) -> Optional[str]:
    ...
    t0 = re.sub(r"[^a-z]", "", t)
    if t0.startswith("positive"): return "positive"
    if t0.startswith("negative"): return "negative"
    return None
...
def build_refinement_messages(remaining_examples: List[Dict[str, str]], num_candidates: int) -> List[Dict[str, str]]:
    ...
    minority = "positive" if counts["positive"] < counts["negative"] else "negative"
    ...
    sys_msg = ("You are improving in-context examples for sentiment classification. "
...
def main():
    ...
    if args.run_random_search:
        ...
        for spec in unique_specs:
            ...
            for idx, seed in enumerate(ordered_seed_list):
                _out_text, init_examples = proposer.generate_examples_balanced(
                    k, max_new_tokens=args.max_new_tokens_proposer, seed=seed
                )
                ...
                this_row_early_stop = 1 if ((1 in seeds_list) and (seed == 1) and (acc < 0.9250)) else 0
                ...
                if this_row_early_stop:
                    print(f"[RANDOM][EARLY-STOP] spec#{spec_id}: seed=1 acc={acc:.4f} < 0.9250 — skipping remaining seeds for this spec.")
                    early_stopped = True
                    break
                    ...
                    for refine_cands in refine_cands_list:
                        for replay_add in replay_add_list:
                            ...
                            for idx, seed in enumerate(ordered_seeds):
                                ...
                                this_row_early_stop = 1 if ((1 in seeds_list) and (seed == 1) and (acc < 0.68)) else 0
                                ...
                                if this_row_early_stop:
                                    print(f"[GRID][EARLY-STOP] spec#{spec_id}: seed=1 acc={acc:.4f} < 0.68 — skipping remaining seeds for this spec.")
                                    early_stopped = True
                                    break
    ...
    out_text, examples = proposer.generate_examples_balanced(
        args.k, max_new_tokens=args.max_new_tokens_proposer, seed=args.seed
    )
    if len(examples) < args.k:
        ...
        print(f"[WARN] Parsed only {len(examples)} / {args.k} examples from proposer output. "
              f"(positive={counts['positive']}, negative={counts['negative']}) Continuing with available.")
